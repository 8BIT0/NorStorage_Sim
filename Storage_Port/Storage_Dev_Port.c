    ...
    for (uint8_t i = 0; i < write_cnt; i ++)
    {
        switch((uint8_t)p_dev->chip_type)
        {
            case Storage_ChipType_W25Qxx:
                ...
                if ((To_DevW25Qxx_API(p_dev->api)->erase_sector(To_DevW25Qxx_OBJ(p_dev->obj), addr_tmp) != SimDevW25Qxx_Ok) || \
                    (To_DevW25Qxx_API(p_dev->api)->write_sector(To_DevW25Qxx_OBJ(p_dev->obj), addr_tmp, p_data, p_dev->sector_size) != SimDevW25Qxx_Ok))
                    return false;
                break;
...
static bool Storage_Dev_Read_Section(StorageDevObj_TypeDef *p_dev, uint32_t addr, uint8_t *p_data, uint16_t len)
{
    ...
    for (uint8_t i = 0; i < read_cnt; i++)
    {
        switch((uint8_t)p_dev->chip_type)
        {
            case Storage_ChipType_W25Qxx:
                /* read sector */
                if (To_DevW25Qxx_API(p_dev->api)->read_sector(To_DevW25Qxx_OBJ(p_dev->obj), addr_tmp, p_data, len) != SimDevW25Qxx_Ok)
                {
                    memset(p_data, 0, len);
                    return false;
                }
    ...
    for (uint16_t i = 0; i < format_size / Storage_TabSize; i++)
    {
        switch (p_dev->chip_type)
        {
            case Storage_ChipType_W25Qxx:
                ...
                if (To_DevW25Qxx_API(p_dev->api)->erase_sector(To_DevW25Qxx_OBJ(p_dev->obj), erase_addr) != SimDevW25Qxx_Ok)
                    return false;
    ...
    while (true)
    {
        switch (p_dev->chip_type)
        {
            case Storage_ChipType_W25Qxx:
                ...
                if (To_DevW25Qxx_API(p_dev->api)->read_sector(To_DevW25Qxx_OBJ(p_dev->obj), section_addr, read_tmp, Storage_TabSize) != SimDevW25Qxx_Ok)
                    return false;
    ...
    switch ((uint8_t)p_dev->chip_type)
    {
        case Storage_ChipType_W25Qxx:
            ...
            while (true)
            {
                ...
                /* erase whole section */
                if (To_DevW25Qxx_API(p_dev->api)->erase_sector(To_DevW25Qxx_OBJ(p_dev->obj), section_addr) != SimDevW25Qxx_Ok)
                    return false;
                ...
                if (To_DevW25Qxx_API(p_dev->api)->write_sector(To_DevW25Qxx_OBJ(p_dev->obj), section_addr, read_tmp, Storage_TabSize) != SimDevW25Qxx_Ok)
                    return false;
